...
int main()
{
    // std::cout << "distance: " << vec_distance({1, 2, 3}, {3, 4, 5}) << "\n";
    srand(0);

    cv::Mat img = cv::imread("../pic/meow.png", cv::IMREAD_UNCHANGED);
    ...
    std::vector<std::vector<float>> ret_vec = replace_label_to_center(data_vec, ret.labels, ret.centers);
